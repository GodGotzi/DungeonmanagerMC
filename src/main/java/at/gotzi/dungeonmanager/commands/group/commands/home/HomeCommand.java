...
import at.gotzi.dungeonmanager.manager.file.SkillManager;
...
public class HomeCommand extends GroupCommand implements TabCompleter {
    ...
    public boolean execute() {
        if (sender instanceof Player player) {
            ...
            if (player.hasPermission("groups.all") || bool) {
                if(args.length == 0) {
                    Home home = PlayerData.getHome(0, player.getUniqueId());
                    if (home != null)
                        TeleportManager.registerTeleport(player, new Location(Bukkit.getWorld(home.getWorld()), home.getX(), home.getY(), home.getZ(), home.getYaw(), home.getPitch()));
                    else
