...
public class TpaCommand extends GroupCommand implements TabCompleter {
        ...
        if(sender instanceof Player player) {
            if (args.length == 1) {
                if (args[0].equalsIgnoreCase("accept")) {
                    ...
                    player.sendMessage(Messages.pTpaAccept);
                    target.sendMessage(Messages.selfTpaAccept);
                    TeleportManager.registerTeleport(target, player.getLocation());
    ...
    private void execute2(Player player, String[] args) {
        ...
        for (Skill sks : skills) {
            if (sks.getSkills() == Skills.TPACMD) {
                bool = true;
                skill = sks;
                break;
            }
        ...
        if (player.hasPermission("groups.all") || bool) {
            if (!coolDown.containsKey(player.getUniqueId())) {
                ...
                TextComponent msg = new TextComponent(Messages.pTpaRequestMsg.replace("%PLAYER%", player.getName()));
                msg.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/tpa accept"));
                msg.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text(Messages.hoverTpaMsg.replace("%PLAYER%", player.getName()))));
                player.sendMessage(Messages.selfTpaRequestMsg.replace("%PLAYER%", target.getName()));
            ...
            player.sendMessage(Messages.needGroup.replace("%CLASS%", GroupManager.getSkillGroups(Skills.TPACMD)).toString());
    }
